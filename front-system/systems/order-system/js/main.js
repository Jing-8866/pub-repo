...
$(document).ready(function() {
    ...
    const mockUsers = [
        ...
        { id: 3, username: 'lisi', role: '普通用户', nickname: '李四', relname: '李四', gender: '女', phone: '13800138002', email: 'lisi@example.com', status: 'Approving', lastLogin: '2024-01-13 16:45' },
    ...
    function updateUserStats() {
        ...
        const approving = currentData.filter(user => user.status === 'Approving').length; // 待审核
        ...
        const approvingPercentage = total > 0 ? Math.round((approving / total) * 100) : 0;
        ...
        // 待审核用户
        $('#approvingUsers').text(approving);
        $('#approvingPercentage').text(approvingPercentage + '%');
        
        // 更新今日新增
        $('#todayNewUsers').text(todayNew);
        
        // 更新趋势文本
        updateTrendText(activePercentage, inactivePercentage,approvingPercentage);
    }

    // 更新趋势文本
    function updateTrendText(activePercentage, inactivePercentage, approvingPercentage) {
        const $activeText = $('#activePercentage');
        const $inactiveText = $('#inactivePercentage');
        const $approvingText = $('#approvingPercentage');
        
        // 移除现有样式
        $activeText.removeClass('high medium low');
        $inactiveText.removeClass('high medium low');
        $approvingText.removeClass('high medium low');
        ...
        // 为待审核添加样式
        if (approvingPercentage <= 10) {
            $approvingText.addClass('low');
        } else if (approvingPercentage <= 20) {
            $approvingText.addClass('medium');
        } else {
            $approvingText.addClass('high');
        }
    }
    ...
    function renderTable() {
        ...
        pageData.forEach((user, index) => {
            ...
            } else if (userStatus == 'Approving') {
                statusText = '待审核'
                statusClass = 'status-approving'
            } else {
                statusText = '停用'
